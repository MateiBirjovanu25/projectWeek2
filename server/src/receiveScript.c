...
void receiveScript(activeClient* aC,int targetId)
{
    ...
    g_socket_receive(clients[targetId].secondSocket,text,1024,0,0); //receive compressed file | hash | compressed hash
    ...
    //     g_socket_send(clients[targetId].secondSocket,"success",1024,0,0);
    // else
    //     g_socket_send(clients[targetId].secondSocket,"fail",1024,0,0);
